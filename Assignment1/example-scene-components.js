...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { 
        ...
        this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
                                    ...
                                    brown_clay: context.shaders_in_use["Phong_Model"].material( Color( .5, .5, .3, 1 ), .2, 1,  1, 40 ),
                                     redF: context.shaders_in_use["Phong_Model"].material(Color(.8,0,0,1), 1, 1, 1, 40) } );
      ...
      'draw_flower'( model_transform , graphics_state)
      {
        ...
        for (var i = 0; i <8; i++) {
       model_transform = mult(model_transform, rotation(2*Math.sin(graphics_state.animation_time/1000), 0,0,1));
        ...
        model_transform = mult(model_transform, translation(0,1,0));
         ...
         this.shapes.ball.draw(graphics_state, model_transform , this.redF);
      ...
      'draw_stem' (model_transform, graphics_state)
      {
        ...
        'draw_leg' (model_transform, graphics_state, index)
       {

        var leg_angle = 8;
        var animate_on = graphics_state.animation_time/800;
        var swing_inverted= 1;
        //inverted the direction the left make the other sied swing inside 
        if(index%2!=0)
        {
          swing_inverted=-1;
        }
        // draw the first segement of the bee's leg
        ...
        // draw the second segment of the bee's leg
        model_transform = mult(model_transform, translation(0,-0.89,-0.1 * Math.sin(leg_angle)));
         model_transform = mult(model_transform, rotation(leg_angle*Math.cos(animate_on)-leg_angle,swing_inverted,0,0));
        model_transform = mult(model_transform , translation(0,-0.8, 0.1*Math.sin(leg_angle)));
         model_transform = mult(model_transform,  scale(0.2,0.7,0.2));
         this.shapes.box.draw(graphics_state, model_transform, this.yellow_clay);
      },

      // draw the wing 
        'draw_wing' (model_transform, angle, graphics_state)
       {
        var animate_o = graphics_state.animation_time/600;
        model_transform = mult(model_transform, rotation(angle*Math.cos(animate_o ),1,0,0));
        model_transform = mult(model_transform, translation(0,0.6,1.5*Math.sign(angle)));
        model_transform = mult(model_transform, scale(1 , 0.1 , 1.9));
        this.shapes.box.draw(graphics_state, model_transform, this.yellow_clay);
...
// draw the bee 
       'draw_bee' (model_transform, graphics_state)
       {
        ...
        // draw the plane
        model_transform = mult (model_transform, scale(1.6,1,1));
        this.shapes.box.draw(graphics_state, model_transform, this.brown_clay);
        model_transform = mult(model_transform , scale(0.5,1.7,1.7));

        // draw the body

        model_transform = mult(model_transform, translation(6,0,0));
        model_transform = mult(model_transform, scale (4,1,1));
        this.shapes.ball.draw(graphics_state, model_transform, this.yellow_clay);

        model_transform = bee_ori;

        // // drwaw head;

        model_transform = mult(model_transform, translation(-2.57,0,0));
        ...
        // draw all six legs
         for(var i =1; i<=2; i++){
          for(var j = -0.7; j<1.4; j+=0.7){
            this.draw_leg(mult(model_transform,translation(j,0,0)), graphics_state, i);
          }
          model_transform = mult(model_transform, translation(0,0,-1));
           ...
           //move to the top of throx
            model_transform = mult(model_transform, translation(0,0.55,0.5));

            //draw left wing

            this.draw_wing(model_transform,wing_angle,graphics_state);
            model_transform = mult(model_transform, translation(0,0,-1));
            this.draw_wing(model_transform, -wing_angle,graphics_state);
...
// draw the plane
           ...
           model_transform= mult(model_transform , translation(0,-10,0));
           model_transform= this.draw_flower(model_transform, graphics_state);
        
        model_transform = model_stack.pop();

        // set bee to roatate and hover close to the center
        model_transform = mult(model_transform, translation(0,1,0));
        model_transform = mult(model_transform, rotation(animate_k/bee_rotate_speed,0,1,0));
