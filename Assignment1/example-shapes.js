  ...
  { 'populate'( using_flat_shading ) // Takes a boolean argument
      {
        ...
        {                                                                 // better, but can't produce flat shading or discontinuous seams in textures.
            this.positions     .push( vec3(0,0,0),    vec3(1,0,0), vec3(0,1,0), vec3(0,0,1), vec3(b,b,0));
            this.normals       .push( vec3(-a,-a,-a), vec3(1,0,0), vec3(0,1,0), vec3(0,0,1),  vec3(b,b,0));
            this.texture_coords.push( vec2(0,0),      vec2(1,0),   vec2(0,1),   vec2(1,b), (b,1)  );
            this.indices.push( 0, 1, 2,   0, 1, 3,   0, 2, 3,    1, 2, 3);                     // Vertices are shared multiple times with this method.
        }
        else
        { this.positions.push( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) );         // Method 2:  A tetrahedron with four independent triangles.
          this.positions.push( vec3(0,0,0), vec3(0,0,1), vec3(1,0,0) );
          this.positions.push( vec3(0,0,0), vec3(0,0,1), vec3(0,1,0) );
          this.positions.push( vec3(0,0,0), vec3(0,1,0), vec3(b,b,0) );
          this.positions.push( vec3(0,0,0), vec3(1,0,0), vec3(b,b,0) );
          ...
          this.normals.push( vec3(-b,-b,0), vec3(-b,-b,0), vec3(-b,-b,0) );
          this.normals.push( vec3(0,-b,-b), vec3(0,-b,-b), vec3(0,-b,-b) );
          this.normals.push( vec3( a,a,a), vec3( a,a,a), vec3( a,a,a) );

          this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(b,b) );    // Each face in Method 2 also gets its own set of texture coords
          this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(b,b) );    //(half the image is mapped onto each face).  We couldn't do this
          this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(b,b) );    // with shared vertices -- after all, it involves different results
          this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(b,b) );
           this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(b,b) );     // when approaching the same point from different directions.
